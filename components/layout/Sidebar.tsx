...
export function Sidebar({ user, isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div
        className={cn(
          'flex flex-col h-[100dvh] fixed left-0 top-0 z-40 bg-[#111111] border-r border-[#2a2a2a] transition-all duration-200 ease-in-out',
          isOpen ? 'w-[240px] translate-x-0' : 'w-[60px] -translate-x-full md:translate-x-0'
        )}
      >
      <div className="flex items-center justify-between px-4 py-4 border-b border-[#2a2a2a] h-12">
        {isOpen && <span className="text-sm font-bold text-white tracking-tight">GoalFlow</span>}
        <button
          onClick={onToggle}
          className={cn(
